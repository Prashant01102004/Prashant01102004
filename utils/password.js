...
module.exports.myPasswordPower = (password) => {
    ...
    if (data.length < 8) {
        ...
        if (!data.contains.includes('lowercase')) {
            return {
                success: false,
                error: "Password should also contain a lowercase (a,b,c...) letter"
            };
        }
        else if (!data.contains.includes('uppercase')) {
            return {
                success: false,
                error: "Password should also contain a Uppercase (A,B,C...) letter"
            };
        }
        else if (!data.contains.includes('number')) {
            return {
                success: false,
                error: "Password should also contain a Number (1,2,3...)"
            };
        }
        else {
            return {
                success: false,
                error: "Password should also contain a symbol(@,#,*...)"
            };
        }
    }
    else return {
